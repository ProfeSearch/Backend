...
const sendToken = (user, name, statusCode, res) => {
...
exports.signup = catchAsync(async (req, res, next) => {
    ...
    sendToken(newUser, name, 201, res);
});
...
exports.login = catchAsync(async (req, res, next) => {
    ...
    if (user.role === 'student') {
        username = (await Student.findOne({ user: user.id })).name;
    } else if (user.role === 'faculty') {
        username = (await Faculty.findOne({ user: user.id })).name;
    }
    sendToken(user, username, 200, res);
});
...
exports.protect = catchAsync(async (req, res, next) => {
    ...
    console.log('Decoded at: ', decoded.iat);
...
//check user status: user, student, faculty
exports.checkStatus = catchAsync(async (req, res, next) => {
    ...
    //grant access to the protected route
    req.user = currentUser;
    if (currentUser.role === 'student') req.identity = 'student';
    else if (currentUser.role === 'faculty') req.identity = 'faculty';
...
exports.updatePassword = catchAsync(async (req, res, next) => {
    ...
    //check if POST password is valid
    if (
